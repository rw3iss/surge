...
export function generateTokens(userId: string, role: UserRole): { accessToken: string; refreshToken: string; expiresAt: Date } {
...
export async function authenticateWithPatreon(
  ...
  const tiers = memberships.flatMap((m) =>
    (m.attributes.currently_entitled_tiers as string[]) || []
  );
