...
import { paginatedQuery, deleteById, PaginationOptions, PaginatedResult } from './base.repo';
...
export async function saveContentBlocks(
  ...
  for (const block of blocks) {
    ...
    await query(
      ...
      [
        postId, block.type, block.sort_order, data,
        data.provider || null, data.url || null,
...
export async function createPost(data: Record<string, unknown>, authorId: string): Promise<PostWithBlocks> {
  ...
  const result = await query(
    ...
    [
      ...
      data.featuredImage, authorId, data.status || 'draft',
      data.isPrivate || false, data.accessLevel || 'public',
      data.tags || [], data.categories || [],
      data.metaTitle, data.metaDescription, publishedAt,
    ]
  );
...
export async function updatePost(id: string, data: Record<string, unknown>): Promise<PostWithBlocks> {
  ...
  const fields: Record<string, string> = {
    ...
    featuredImage: 'featured_image', status: 'status', isPrivate: 'is_private',
    accessLevel: 'access_level', tags: 'tags', categories: 'categories',
