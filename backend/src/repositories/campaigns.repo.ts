...
export async function createCampaign(data: Record<string, unknown>, userId: string): Promise<Campaign> {
  const result = await query(
    `INSERT INTO campaigns (title, slug, description, short_description, featured_image,
     ...
     RETURNING *`,
    [
      data.title, data.slug, data.description, data.shortDescription,
      data.featuredImage, data.goalAmountCents, data.status || 'draft',
      data.startDate, data.endDate, data.isPublished || false, userId,
    ]
  );
...
export async function getDonationSummary(): Promise<DonationSummary> {
  const [totalAll, totalMonth, totalYear, breakdown, general] = await Promise.all([
    ...
    query(`SELECT COALESCE(SUM(amount_cents), 0) as total FROM donations WHERE status = 'completed' AND created_at >= date_trunc('month', CURRENT_DATE)`),
    query(`SELECT COALESCE(SUM(amount_cents), 0) as total FROM donations WHERE status = 'completed' AND created_at >= date_trunc('year', CURRENT_DATE)`),
    ...
    query(`SELECT COALESCE(SUM(amount_cents), 0) as total FROM donations WHERE campaign_id IS NULL AND status = 'completed'`),
  ]);
