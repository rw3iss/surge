...
router.post('/webhook', async (req: Request, res) => {
  ...
  try {
    ...
    if (config.stripe.webhookSecret) {
      ...
      logger.warn('STRIPE_WEBHOOK_SECRET is not set - skipping webhook signature verification (development mode)');
...
router.get('/admin/user/:userId/transactions', authenticate(), requireAdmin, async (req: AuthenticatedRequest, res) => {
  try {
