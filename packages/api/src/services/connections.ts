...
export function sanitizeCredentials(
    ...
    // X OAuth 1.0a: consumer key masked dots+last6 (X-style), secrets fully masked.
    if (sanitized.apiKey) {
        const key = String(sanitized.apiKey,);
        sanitized.apiKey = '••••••••••' + key.slice(-6,);
        sanitized.hasApiKey = true;
    }
