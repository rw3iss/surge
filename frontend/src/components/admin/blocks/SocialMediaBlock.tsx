import { Component, Show, createResource, For, createSignal } from 'solid-js';
    ...
    <div class="block-social-media">
      <Show when={props.mode === 'edit'} fallback={
        <div class="block-social-media__preview">
          <Show when={props.data.postUrl} fallback={<span class="block-text__empty">No social media post selected. Click Edit to choose one.</span>}>
            <div class="block-social-media__selected">
              ...
              <div class="block-social-media__details">
                ...
                <p>{props.data.content?.substring(0, 120)}{props.data.content?.length > 120 ? '...' : ''}</p>
              </div>
            </div>
          </Show>
        </div>
        ...
        <Show when={selectedProvider() && posts()}>
          <div class="form-group">
            ...
            <div class="social-media-grid">
              <For each={posts()} fallback={<div class="empty-state">No posts found for this provider.</div>}>
                {(post: any) => (
                  <div
                    class={`social-media-grid__item ${props.data.postId === post.id ? 'social-media-grid__item--selected' : ''}`}
        ...
        <div class="form-group">
          <label class="checkbox-label">
            <input type="checkbox" checked={props.data.showComments || false} onChange={(e) => props.onUpdate({ ...props.data, showComments: e.currentTarget.checked })} />
            Show comments
          </label>
        </div>
      </Show>
