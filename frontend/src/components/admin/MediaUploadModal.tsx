...
export default function MediaUploadModal(props: MediaUploadModalProps) {
  ...
  return (
    <div class="media-upload-overlay" onClick={(e) => { if (e.target === e.currentTarget) props.onClose(); }}>
      <div class="media-upload-modal">
          ...
          <Show when={file()}>
            <div class="media-upload-modal__preview">
              ...
              <div class="media-upload-modal__file-info">
                ...
                <span class="media-upload-modal__file-meta">{formatSize(file()!.size)} &middot; {file()!.type}</span>
              </div>
            </div>
            ...
            <div class="media-upload-modal__fields">
              <div class="form-group">
                ...
                <input type="text" value={title()} onInput={(e) => setTitle(e.currentTarget.value)} placeholder="Media title" />
              </div>
              <div class="form-group">
                <label>Description (optional)</label>
                <textarea rows={2} value={description()} onInput={(e) => setDescription(e.currentTarget.value)} placeholder="Brief description..." />
              </div>
            </div>
          </Show>
        </div>
        ...
        <div class="media-upload-modal__footer">
          ...
          <Show when={!uploading() && !success()}>
            ...
            <button type="button" class="btn btn--primary" onClick={handleUpload} disabled={!file()}>Upload</button>
          </Show>
        </div>
      </div>
    </div>
