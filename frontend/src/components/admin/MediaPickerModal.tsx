...
const MediaPickerModal: Component<MediaPickerModalProps> = (props) => {
  ...
  return (
    <div class="media-picker-overlay" onClick={(e) => { if (e.target === e.currentTarget) props.onClose(); }}>
      <div class="media-picker">
        <div class="media-picker__header">
          <h2>Select Existing {props.type === 'image' ? 'Image' : props.type === 'video' ? 'Video' : 'Document'}</h2>
            ...
            <Show when={(media() || []).length > 0} fallback={
              ...
              <div class={`media-picker__grid media-picker__grid--${props.type}`}>
                <For each={media()}>
                  {(item: MediaItem) => (
                    <div
                      class={`media-picker__item ${selected() === item.id ? 'media-picker__item--selected' : ''}`}
