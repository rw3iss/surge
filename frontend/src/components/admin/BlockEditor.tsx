...
const BlockEditor: Component<BlockEditorProps> = (props) => {
  ...
  const handleDragStart = (e: PointerEvent, id: string) => {
    ...
    const handleMove = (moveEvt: PointerEvent) => {
      ...
      newIndex = Math.max(0, Math.min(currentBlocks.length - 1, newIndex > currentIndex ? newIndex - 1 : newIndex));
  ...
  return (
    <div class="block-editor">
      <div class={`content-blocks-list ${draggingId() ? 'content-blocks-list--dragging' : ''}`}>
        <Index each={props.blocks} fallback={<div class="empty-state">No content blocks yet. Add one below.</div>}>
          {(block, index) => (
            <ContentBlock
              block={block()}
              index={index}
