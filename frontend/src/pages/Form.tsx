import { Component, createResource, createSignal, For, Show, Switch, Match } from 'solid-js';
import { useParams } from '@solidjs/router';
import { Title, Meta, Link } from '@solidjs/meta';
import { fetchForm, submitForm, fetchFormResults } from '../services/api';
import type { Form, FormQuestion, FormResults, QuestionResult, ChoiceSummary, TextSummary, NumberSummary } from '@surge/shared';
                ...
                <For each={f().questions}>
                  {(q: FormQuestion) => (
                    <div class="form-field">
                      ...
                      {q.type === 'number' && <input type="number" required={q.isRequired} onInput={(e) => updateAnswer(q.id, Number(e.currentTarget.value))} />}
                      {q.type === 'email' && <input type="email" required={q.isRequired} onInput={(e) => updateAnswer(q.id, e.currentTarget.value)} />}
                      {q.type === 'date' && <input type="date" required={q.isRequired} onInput={(e) => updateAnswer(q.id, e.currentTarget.value)} />}
                      {q.type === 'select' && (
                        <select required={q.isRequired} onChange={(e) => updateAnswer(q.id, e.currentTarget.value)}>
                ...
                <Show when={results()}>
                  {(r) => (
                    <div class="form-results">
                      <h2>Results</h2>
                      <p class="form-results__count">{r().totalSubmissions} total submission{r().totalSubmissions !== 1 ? 's' : ''}</p>
                      <For each={r().questionResults}>
                        {(qr: QuestionResult) => (
                          <div class="form-results__question">
                            <h3>{qr.question}</h3>
                            <p class="form-results__responses">{qr.responses} response{qr.responses !== 1 ? 's' : ''}</p>
                            <Switch>
                                  ...
                                  <For each={(qr.summary as ChoiceSummary).options}>
                                    {(option) => (
                                      <div class="result-bar">
                                        <div class="result-bar__label">{option.value}</div>
                                        <div class="result-bar__track">
                                          <div class="result-bar__fill" style={`width: ${option.percentage}%`} />
                                        </div>
                                        <div class="result-bar__value">{Math.round(option.percentage)}%</div>
                                      </div>
                                    )}
                                  </For>
                                </div>
                                  ...
                                  return (
                                    <div class="form-results__stats">
                                      <div class="form-results__stat">
                                        <span class="form-results__stat-label">Min</span>
                                        <span class="form-results__stat-value">{s.min}</span>
                                      </div>
                                      <div class="form-results__stat">
                                        <span class="form-results__stat-label">Max</span>
                                        <span class="form-results__stat-value">{s.max}</span>
                                      </div>
                                      <div class="form-results__stat">
                                        <span class="form-results__stat-label">Average</span>
                                        <span class="form-results__stat-value">{s.average.toFixed(1)}</span>
                                      </div>
                                      <div class="form-results__stat">
                                        <span class="form-results__stat-label">Median</span>
                                        <span class="form-results__stat-value">{s.median}</span>
                                      </div>
                                    </div>
                                  );
                                })()}
                              ...
                              <Match when={qr.summary.type === 'text'}>
                                <p class="form-results__text-count">
                                  {(qr.summary as TextSummary).totalResponses} response{(qr.summary as TextSummary).totalResponses !== 1 ? 's' : ''}
                                </p>
                              </Match>
                            </Switch>
                          </div>
