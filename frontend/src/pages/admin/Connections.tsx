            ...
            return (
              <div class={`connection-card ${conn() ? 'connection-card--connected' : ''}`}>
                ...
                <div class="connection-card__actions">
                  <Show when={conn()}>
                    <button class="btn btn--small btn--icon" onClick={() => moveConnection(provider.id, 'up')} disabled={index() === 0} title="Move up">&#9650;</button>
                    <button class="btn btn--small btn--icon" onClick={() => moveConnection(provider.id, 'down')} disabled={index() === PROVIDERS.length - 1} title="Move down">&#9660;</button>
                    <A href={`/admin/connections/${provider.id}`} class="btn btn--small btn--secondary">Edit</A>
                    <button class="btn btn--small btn--danger" onClick={() => handleDisconnect(provider.id)}>Disconnect</button>
                  </Show>
                  <Show when={!conn()}>
                    <A href={`/admin/connections/${provider.id}`} class="btn btn--small btn--primary">Connect</A>
                  </Show>
                </div>
              </div>
            );
