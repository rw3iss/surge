...
const AdminMedia: Component = () => {
  ...
  const handleModalContentClick = (m: any) => {
    ...
    if (m.mimeType?.startsWith('image/') || (!m.mimeType?.startsWith('video/') && !m.mimeType?.startsWith('audio/'))) {
    ...
    <div>
      ...
      <div class="admin-header">
        ...
        <label class="btn btn--primary">
          Upload File
          <input type="file" onChange={handleUpload} accept="image/*,video/*,audio/*,application/pdf,.doc,.docx,.zip" style={{ display: 'none' }} />
        </label>
      </div>

      <div class="media-filters" style={{ display: 'flex', gap: '0.75rem', "margin-bottom": '1.5rem', "flex-wrap": 'wrap', "align-items": 'center' }}>
      ...
      <Show when={media()?.length} fallback={
            ...
            {(m: any) => (
              <div class="media-grid__item" onClick={() => openModal(m)}>
                <div class="media-grid__preview">
                  ...
                  <Show when={!m.mimeType?.startsWith('image/') && !m.mimeType?.startsWith('video/')}>
                ...
                <Show when={editingId() === m.id} fallback={
                  <div class="media-grid__info">
                    ...
                    <div class="media-grid__actions">
                      <button class="btn btn--small btn--secondary" onClick={(e) => startEdit(m, e)}>Edit</button>
                      <button class="btn btn--small btn--secondary" onClick={(e) => handleDownload(m, e)} title="Download">
                        &#8595;
                      </button>
                      <button class="btn btn--small btn--danger" onClick={(e) => handleDelete(m.id, e)}>Delete</button>
                    </div>
                  </div>
                }>
                  <div class="media-grid__edit" onClick={(e) => e.stopPropagation()}>
                    ...
                    <div class="media-grid__edit-actions">
                      <button class="btn btn--small btn--primary" onClick={saveEdit} disabled={saving()}>
                        {saving() ? 'Saving...' : 'Save'}
                      </button>
          ...
          <div class="media-modal" onClick={handleBackdropClick}>
            <div class="media-modal__container">
              ...
              <div class="media-modal__content" onClick={() => handleModalContentClick(m())}>
                ...
                <Show when={!m().mimeType?.startsWith('image/') && !m().mimeType?.startsWith('video/') && !m().mimeType?.startsWith('audio/')}>
              ...
              <div class="media-modal__footer">
                ...
                <div class="media-modal__meta">
                  ...
                  <span class="media-modal__meta-details">
                    {getTypeLabel(m().mimeType)} &middot; {formatSize(m().size)} &middot; {new Date(m().createdAt).toLocaleDateString()}
                  </span>
                </div>
                <button class="btn btn--primary" onClick={(e) => handleDownload(m(), e)}>Download</button>
              </div>
            </div>
          </div>
        )}
