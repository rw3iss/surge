...
function ConnectionsPanel() {
  ...
  const getConnection = (providerId: string) =>
    connections()?.find((c: any) => c.provider === providerId);
    ...
    <div class="connections-list">
      ...
      <For each={PROVIDERS}>
        {(provider) => {
          ...
          return (
            <div class={`connection-card ${conn() ? 'connection-card--connected' : ''} ${isEditing() ? 'connection-card--editing' : ''}`}>
              <div class="connection-card__row">
                ...
                <div class="connection-card__info">
                  ...
                  <div class="connection-card__status">
                    <Show when={conn()} fallback={<span class="badge badge--muted">Not connected</span>}>
                      <span class="badge badge--success">Connected</span>
                      <Show when={conn()?.autoPublish}>
                        <span class="connection-card__auto-publish">
                          Auto-publish: {conn()?.autoPublishCount ? `last ${conn()?.autoPublishCount}` : 'all'}
                        </span>
                      </Show>
                    </Show>
                  </div>
                ...
                <div class="connection-card__actions">
                  <Show when={!isEditing()}>
                    <button class="btn btn--small btn--secondary" onClick={() => startEditing(provider.id)}>
                      {conn() ? 'Edit' : 'Connect'}
                    </button>
                    <Show when={conn()}>
                      <button class="btn btn--small btn--danger" onClick={() => handleDisconnect(provider.id)}>
                        Disconnect
                      </button>
                    </Show>
                  </Show>
              ...
              <Show when={isEditing()}>
                <div class="connection-card__editor">
                  ...
                  <div class="form-group">
                    ...
                    <input type="password" value={accessToken()} onInput={(e) => setAccessToken(e.currentTarget.value)} placeholder="Paste access token" />
                    <span class="form-help">OAuth access token for {provider.name} API</span>
                  </div>
                  <div class="form-group">
                    <label>API Key (optional)</label>
                    <input type="password" value={apiKey()} onInput={(e) => setApiKey(e.currentTarget.value)} placeholder="API key if required" />
                  </div>
                  <div class="form-group">
                  ...
                  <div class="form-group">
                    <label class="checkbox-label">
                      <input type="checkbox" checked={autoPublish()} onChange={(e) => setAutoPublish(e.currentTarget.checked)} />
                  ...
                  <Show when={autoPublish()}>
                    <div class="form-group" style={{ "padding-left": "1.5rem" }}>
                      <label class="checkbox-label">
                        <input type="checkbox" checked={publishAll()} onChange={(e) => { setPublishAll(e.currentTarget.checked); if (e.currentTarget.checked) setAutoPublishCount(null); }} />
                        <span>Publish all posts</span>
                      </label>
                      <Show when={!publishAll()}>
                        <div style={{ "margin-top": "0.5rem" }}>
                          <label>Number of recent posts</label>
                          <input type="number" min="1" max="100" value={autoPublishCount() || ''} onInput={(e) => setAutoPublishCount(parseInt(e.currentTarget.value) || null)} style={{ width: "100px" }} />
                        </div>
                      </Show>
                    </div>
                  </Show>
                  <div class="form-actions" style={{ "margin-top": "1rem" }}>
                    <button class="btn btn--primary btn--small" onClick={handleSaveConnection}>Save</button>
                    <button class="btn btn--secondary btn--small" onClick={cancelEditing}>Cancel</button>
                  </div>
                </div>
              </Show>
            </div>
        ...
        <CollapsibleSection title="General" defaultOpen={true}>
          <form onSubmit={handleSubmit}>
            <div class="form-group">
              ...
              <input type="text" value={siteName()} onInput={(e) => setSiteName(e.currentTarget.value)} />
            </div>
            <div class="form-group">
              <label>Site Description</label>
              <input type="text" value={siteDescription()} onInput={(e) => setSiteDescription(e.currentTarget.value)} />
            </div>
            <div class="form-group">
              <label>Contact Email</label>
              <input type="email" value={contactEmail()} onInput={(e) => setContactEmail(e.currentTarget.value)} />
            </div>

            <h3 style={{ "margin-top": "1.5rem", "margin-bottom": "0.75rem", "font-size": "0.9rem", "font-weight": "600", "color": "#666" }}>Integrations</h3>
            <div class="form-group">
              <label>Google Analytics ID</label>
              <input type="text" value={analyticsId()} onInput={(e) => setAnalyticsId(e.currentTarget.value)} />
