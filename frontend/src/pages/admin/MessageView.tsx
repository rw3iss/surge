...
const AdminMessageView: Component = () => {
  ...
  const statusBadge = (status: string) => {
    switch (status) {
      case 'unread': return 'badge--info';
      case 'read': return 'badge--muted';
          ...
          <div class="admin-form">
            <div class="form-section">
              ...
              <div style={{ display: 'grid', "grid-template-columns": '1fr 1fr', gap: '1rem', "margin-bottom": '1.5rem' }}>
                <div>
                  <label style={{ "font-weight": '600', "font-size": '0.85rem', color: '#64748b' }}>From</label>
                  <p style={{ margin: '0.25rem 0 0' }}>{m().name}</p>
                </div>
                <div>
                  <label style={{ "font-weight": '600', "font-size": '0.85rem', color: '#64748b' }}>Email</label>
                  <p style={{ margin: '0.25rem 0 0' }}>
                    <a href={`mailto:${m().email}`}>{m().email}</a>
                  </p>
                </div>
                <div>
                  <label style={{ "font-weight": '600', "font-size": '0.85rem', color: '#64748b' }}>Date</label>
                  <p style={{ margin: '0.25rem 0 0' }}>{new Date(m().createdAt).toLocaleString()}</p>
                </div>
                <div>
                  <label style={{ "font-weight": '600', "font-size": '0.85rem', color: '#64748b' }}>IP Address</label>
                  <p style={{ margin: '0.25rem 0 0' }}>{m().ipAddress || '—'}</p>
                </div>
              </div>

              <div style={{ "margin-bottom": '1.5rem' }}>
                <label style={{ "font-weight": '600', "font-size": '0.85rem', color: '#64748b' }}>Message</label>
                <div style={{
                  "margin-top": '0.5rem',
                  padding: '1rem',
                  background: '#f8fafc',
